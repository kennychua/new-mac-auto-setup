#export PERL5LIB=/opt2/its/perllib/custom:/opt2/its/perllib/lib/perl5/site_perl/5.8.5/:/opt2/its/perllib/lib64/perl5/site_perl/5.8.5:/opt2/its/perllib:/opt2/its/perllib/custom/lib/perl5/5.8.5/
#export LD_LIBRARY_PATH=/opt2/its/var/openssl/lib:/opt2/its/var/freetds/lib
#export PATH=$PATH:~/bin:/opt2/its/bin:/opt2/its/bin/bin
export EDITOR=vim
#export MANPATH=$MANPATH:/opt2/its/share/man
...
alias ezdiff='diff -yr --suppress-common-lines'
alias ezgrep='grep -Ri --exclude='*svn-base*''
#alias ct='~/ctags'
alias rm='rm -i'
alias cp='cp -i'
alias mv='mv -i'
...
#alias ls='ls -AlaF --color=always' # add colors and file type extensions
#alias lx='ls -lXB'              # sort by extension
#alias lk='ls -lSr'              # sort by size
#alias lc='ls -lcr'      # sort by change time
#alias lu='ls -lur'      # sort by access time
#alias lr='ls -lR'               # recursive ls
alias ls='ls -ltrGp'              # sort by date
...
alias moew='more'
alias kk='ll'
alias al='ls -Al'               # show hidden files
alias sl='ls -AlaF --color=always' # add colors and file type extensions
#----------------
# a few fun ones
#----------------
...
#
# prompt color setup 
# (using escape sequences, see format specification below)
#
normalfg="[0m"
greenfg="[0;32m"
yellowfg="[0;33m"
...
redfg="[1;31m"  # bright
 ...
 sequences like \[ and \] are necessary to surround 'escape sequences'
 using them bash computes corect line width

if [ $HOST = "mygreenhost" ]; then
    PS1="\[^[${greenfg}\]${PS1}\[^[${normalfg}\]"
fi
if [ $HOST = "myhost" ]; then
    PS1="\[^[${redfg}\]${PS1}\[^[${normalfg}\]"
fi
if [ $HOST = "mybluehost" ]; then
    PS1="\[^[${bluefg}\]${PS1}\[^[${normalfg}\]"
fi
...
let promptsize=$(echo -n "--($user@$host ddd, DD mmm YY)---(${PWD})---" \
                 | wc -c | tr -d " ")
let fillsize=${COLUMNS}-${promptsize}-15
fill=""
while [ "$fillsize" -gt "0" ] 
do 
    fill="${fill}-"
...
function listening {
    ...
    if [ "${1}" = "-h" ]; then
        echo "Usage: openfiles [r|w|R|W] regex"
        return
    fi
    if [ "$#" = "0" ]; then
    ...
    OPENFILES=$(sudo lsof -PXn -p ${PIDS} | egrep "[0-9]+${MODE}[A-Za-z]* +REG" | awk '{print $9}' | egrep -v "^[^/]" | sort | uniq);
    if [ "${OPENFILES}" = "" ]; then
        echo "No files opened for ${ACTION}."
    else
        echo "Files opened for ${ACTION}:"
